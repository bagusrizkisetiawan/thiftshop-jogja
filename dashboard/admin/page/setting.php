...
if (!empty($_SESSION['login_admin'])) {
    ...
    if (isset($_POST['simpan'])) {
        ...
        if (!empty($cek_no_hp_user)) {
            $q_data_foto=mysqli_query($koneksi, "SELECT * FROM user WHERE id_user='$id_user_login'");
            $cek_data_foto=mysqli_num_rows($q_data_foto);
            if (!empty($cek_data_foto)) {
                $data_foto=mysqli_fetch_array($q_data_foto);
            }
