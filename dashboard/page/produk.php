...
if (!empty($id_user_login)) {
    ...
    if ($proses=='terjual') {
        $terjual=mysqli_query($koneksi, "UPDATE produk SET status='terjual' WHERE id_produk='$id_produk'");
        if ($terjual) {
        ...
        <div class="table-responsive-lg ">
            <table class="table table-hover my-2" id="datatable">
                ...
                if(!empty($id_kategori)){
                    $q=mysqli_query($koneksi, "SELECT * FROM produk WHERE id_user='$id_user_login' and id_kategori='$id_kategori' order by id_produk DESC");
                }elseif(!empty($status)){
                    $q=mysqli_query($koneksi, "SELECT * FROM produk WHERE id_user='$id_user_login' and status='$status' order by id_produk DESC");
                }elseif(!empty($tinjauan)){
                    $q=mysqli_query($koneksi, "SELECT * FROM produk WHERE id_user='$id_user_login' and tinjauan='$tinjauan' order by id_produk DESC");
                }else{
                    $q=mysqli_query($koneksi, "SELECT * FROM produk WHERE id_user='$id_user_login' order by id_produk DESC");
                }
                ...
                ?>
                    <tr>
                        ...
                        <td >
                            ...
                            <!-- edit -->
                            <a href="?page=edit_produk&&id_produk=<?php echo$data['id_produk'] ?>" class="btn btn-sm mb-1 btn-aksi btn-warning"><i class="fa-solid fa-pen"></i></a>
                            <!-- terjual -->
                            <a href="" class="btn btn-sm mb-1 btn-aksi <?php if($data['status']=='terjual'){echo'disabled btn-secondary';}else{echo'btn-success';} ?> " data-bs-toggle="modal" data-bs-target="#terjual<?php echo$data['id_produk'] ?>"><i class="fa-solid fa-check"></i></a>
                            <div class="modal fade" id="terjual<?php echo$data['id_produk'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                    <div class="modal-body">
                                        <div class="card">
                                            <div class="card-body">
                                                <h6 class=" mb-3 mb-0">Apakah produk ini sudah terjual ?</h6>
                                                <hr>
                                                <div class="row">
                                                    <div class="col-4">
                                                        <div class="img-modal">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="?page=produk&&proses=terjual&&id_produk=<?php echo$data['id_produk'] ?>" class="btn btn-success">Terjual</a>
                                    </div>
                                    </div>
                                </div>
                            </div>
